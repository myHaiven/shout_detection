...
def smooth_softmax(x):
    ...
    return tf.sigmoid(x) / tf.expand_dims(tf.reduce_sum(input_tensor=tf.sigmoid(x), axis=1), axis=1)


def softmax(x, b):

    return tf.exp(b*x) / tf.expand_dims(tf.reduce_sum(input_tensor=tf.exp(b*x), axis=1), axis=1)
...
def affine_transform(x, output_dim, seed=0, name=None):
    ...
    """
    initializer = tf.compat.v1.truncated_normal_initializer(stddev=0.02, seed=seed)

    # weights = tf.get_variable(name + "_w", [x.get_shape()[1], output_dim],
    #                           initializer=tf.contrib.layers.xavier_initializer(seed=seed))
    weights = tf.compat.v1.get_variable(name + "_w", [x.get_shape()[1], output_dim],
                              initializer=initializer)
    b = tf.compat.v1.get_variable(name + "_b", [output_dim], initializer=tf.compat.v1.constant_initializer(0.0))
...
def get_glimpse(inputs, bp, reuse=None, is_training=True):
    ...
    with tf.compat.v1.variable_scope("glimpse_net", reuse=reuse):
...
def multinomial_pmf(mean, sample):
    ...
    """
    p_br = tf.reduce_prod(input_tensor=tf.pow(mean, sample), axis=2)
    return p_br
...
def summary_generation(eval_file_dir):
    ...
    summary_dic["summary_ph"] = summary_ph = tf.compat.v1.placeholder(dtype=tf.float32)

    for name in noise_list:

        with tf.compat.v1.variable_scope(name):
            for summary_name in summary_list:
                    summary_dic[name+"_"+summary_name] = tf.compat.v1.summary.scalar(summary_name, summary_ph)

    with tf.compat.v1.variable_scope("Averaged_Results"):

        summary_dic["cost_across_all_noise_types"] = tf.compat.v1.summary.scalar("cost_across_all_noise_types", summary_ph)
        summary_dic["accuracy_across_all_noise_types"]\
            = tf.compat.v1.summary.scalar("accuracy_across_all_noise_types", summary_ph)
        summary_dic["variance_across_all_noise_types"]\
            = tf.compat.v1.summary.scalar("variance_across_all_noise_types", summary_ph)
    return summary_dic
...
class Model(object):
    ...
    def __init__(self, batch_size, reuse=None, is_training=True):
        ...
        self.keep_probability = tf.compat.v1.placeholder(tf.float32, name="keep_probabilty")
        self.inputs = tf.compat.v1.placeholder(tf.float32, shape=[None, bdnn_inputsize],
                                              name="inputs")
        self.labels = tf.compat.v1.placeholder(tf.float32, shape=[None, bdnn_outputsize], name="labels")
        self.is_training = is_training
        self.mean_bps = []
        self.sampled_bps = []
        self.baselines = []
        self.global_step = tf.Variable(0, trainable=False)
        self.lr = tf.compat.v1.train.exponential_decay(initLr, self.global_step, lrDecayFreq, lrDecayRate, staircase=True)
        self.raw_reward = 0
        # set inference graph
    ...
    def inference(self, reuse=None):
        ...
        tf.compat.v1.set_random_seed(SEED)  # initialize the random seed at graph level
        ...
        for time_step in range(nGlimpses):
            ...
            if time_step == 0:
                with tf.compat.v1.variable_scope("core_network", reuse=reuse_recurrent):
            ...
            if time_step != nGlimpses - 1:  # not final time_step
                ...
                with tf.compat.v1.variable_scope("baseline", reuse=reuse_recurrent):
    ...
    def get_next_input(self, cell_output, reuse=None):
        ...
        with tf.compat.v1.variable_scope("baseline", reuse=reuse):

            baseline = tf.sigmoid(affine_transform(((cell_output)), 1, name='baseline'))

            self.baselines.append(baseline)

        with tf.compat.v1.variable_scope("selection_network", reuse=reuse):
            ...
            if is_training:
                sampled_bp = tf.random.categorical(logits=mean_bp, num_samples=1, seed=SEED)
    ...
    def bdnn_prediction(self, logits, threshold):
        ...
        result, soft_result = tf.compat.v1.py_func(bdnn_prediction, [batch_size_tensor, logits, th_tenor], Tout=[tf.float32, tf.float32])
    ...
    def calc_reward(self, outputs):
        ...
        b = tf.tile(b, [1, 1, 1])
        b = tf.reshape(tf.transpose(a=b, perm=[1, 0, 2]), [batch_size, nGlimpses])
        ...
        mean_bps = tf.transpose(a=mean_bps, perm=[1, 0, 2])
        ...
        sampled_bps = tf.transpose(a=sampled_bps, perm=[1, 0, 2])
        ...
        reward = tf.reduce_mean(input_tensor=R)
        ...
        rf_part = tf.math.log(p_bps + SMALL_NUM) * (R - no_grad_b)

        # J = sv_part

        J = tf.concat(axis=1, values=[sv_part, rf_part])  # comment for sv only
        J = tf.reduce_sum(input_tensor=J, axis=1)
        J = J - tf.reduce_mean(input_tensor=tf.square(R - b), axis=1)  # comment for sv only
        J = tf.reduce_mean(input_tensor=J, axis=0)

        # cost = -J

        cost = -tf.reduce_mean(input_tensor=J)
        var_list = tf.compat.v1.trainable_variables()
        grads = tf.gradients(ys=cost, xs=var_list)
        ...
        return cost, reward, train_op, tf.reduce_mean(input_tensor=b), tf.reduce_mean(input_tensor=R - b), \
               sampled_bps, tf.reduce_mean(input_tensor=p_bps), self.lr, soft_result, raw_labels
...
def main(prj_dir=None, model=None, mode=None):
    ...
    summary_ph = tf.compat.v1.placeholder(dtype=tf.float32)

    with tf.compat.v1.variable_scope("Training_procedure"):

        cost_summary_op = tf.compat.v1.summary.scalar("cost", summary_ph)
        accuracy_summary_op = tf.compat.v1.summary.scalar("accuracy", summary_ph)
    ...
    if mode is 'train':
        train_summary_writer = tf.compat.v1.summary.FileWriter(logs_dir + '/train/', sess.graph, max_queue=2)
        valid_summary_writer = tf.compat.v1.summary.FileWriter(logs_dir + '/valid/', max_queue=2)
    ...
    if ckpt and ckpt.model_checkpoint_path:  # model restore
        ...
        sess.run(tf.compat.v1.global_variables_initializer())  # if the checkpoint doesn't exist, do initialization
